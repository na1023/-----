...
const Portfolio = (() => {
    ...
    [...trades]
      ...
      .forEach(t => {
        ...
        if (t.side === 'buy') {
          ...
          if (h.shares > 0) {
            const ratio  = Math.min(t.shares / h.shares, 1);
  ...
  function sharesAtDateByAccount(trades, symbolCode, exDate) {
    ...
    [...trades]
      ...
      .forEach(t => {
        ...
        acc[a] = (acc[a] ?? 0) + (t.side === 'buy' ? t.shares : -t.shares);
      });
    return acc;
  }
  ...
  function getRealizedPnL(trades) {
    ...
    [...trades]
      ...
      .forEach(t => {
        ...
        if (t.side === 'buy') {
          lots[code].push({ shares: t.shares, costPerShare: Math.abs(t.amount) / t.shares });
          yearly[year].buyAmt += Math.abs(t.amount);
        } else {
          let remaining = t.shares, costBasis = 0;
  ...
  return { getHoldings, calcDividendsReceived, annualDivPerShare, getRealizedPnL, isValidCode };
})();
